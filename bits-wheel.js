(function(){
  ...
  const KEY="tw_bits_wheel_v1";
  const defaults={enabled:false,bitsPerTier:100,spinsPerTier:1,accumulate:true,maxSpins:20,spinDuration:8,gapSeconds:3,displayMode:"on_bits",duringGiveaway:"wait",hideDelay:8,sendChat:true,summary:true,history:true,repeat:true,prizes:[{id:"p1",name:"Ancient Box",chance:25,color:"#9146ff",active:true},{id:"p2",name:"10x Talismã",chance:25,color:"#00e5ff",active:true},{id:"p3",name:"Garuda Feather",chance:20,color:"#00e676",active:true},{id:"p4",name:"Prêmio Surpresa",chance:15,color:"#ffca28",active:true},{id:"p5",name:"Tente novamente",chance:15,color:"#e91e63",active:true}],queue:[],history:[],remainders:{},processedIds:[],paused:false,lastCompleted:null};
  let data=load(),processing=false,current=null,spinTimer=null;
  function load(){try{return {...defaults,...JSON.parse(localStorage.getItem(KEY)||"{}")};}catch(_){return structuredClone(defaults)}}
  function save(){localStorage.setItem(KEY,JSON.stringify(data));window.salvarNuvem?.();renderAll();window.dispatchEvent(new CustomEvent("megaloot:bits-wheel-state"));const pub=window.publishMegaLootOverlayState?.(true);const st=$("bw-sync-status");if(st){st.textContent=`Estado enviado: ${current?.phase||"espera"} · ${new Date().toLocaleTimeString("pt-BR")}`;}return pub;}
  ...
  function settingsFromUI(){
    data.enabled=!!$("bw-enabled")?.checked;data.bitsPerTier=Math.max(1,Number($("bw-bits-tier")?.value||100));data.spinsPerTier=Math.max(1,Number($("bw-spins-tier")?.value||1));data.accumulate=!!$("bw-accumulate")?.checked;data.maxSpins=Math.max(1,Number($("bw-max-spins")?.value||20));data.spinDuration=Math.max(3,Number($("bw-duration")?.value||8));data.gapSeconds=Math.max(0,Number($("bw-gap")?.value||3));data.displayMode=$("bw-display-mode")?.value||"on_bits";data.duringGiveaway=$("bw-during-giveaway")?.value||"wait";data.hideDelay=Math.max(1,Number($("bw-hide-delay")?.value||8));data.sendChat=!!$("bw-send-chat")?.checked;data.summary=!!$("bw-summary")?.checked;data.history=!!$("bw-history-enabled")?.checked;data.repeat=!!$("bw-repeat")?.checked;
  }
  function showTab(name,btn){document.querySelectorAll(".bw-pane").forEach(x=>x.classList.remove("active"));document.querySelectorAll(".bw-tab").forEach(x=>x.classList.remove("active"));$("bw-pane-"+name)?.classList.add("active");btn?.classList.add("active");renderAll()}
  function buildPanel(){
    const card=document.createElement("div");card.className="config-card bw-launch-card";card.innerHTML='<label>🎡 Roleta de Bits</label><span class="config-desc">Giros automáticos por Cheers, brindes manuais, fila e histórico.</span><button class="btn-purple" onclick="openModal(\'bits-wheel-modal\')">ABRIR PAINEL DA ROLETA</button>';
    document.querySelector(".theme-launch-card")?.after(card);
  ...
  function syncUI(){[["bw-enabled","enabled"],["bw-accumulate","accumulate"],["bw-repeat","repeat"],["bw-send-chat","sendChat"],["bw-summary","summary"],["bw-history-enabled","history"]].forEach(([i,k])=>{if($(i))$(i).checked=!!data[k]});[["bw-bits-tier","bitsPerTier"],["bw-spins-tier","spinsPerTier"],["bw-max-spins","maxSpins"],["bw-duration","spinDuration"],["bw-gap","gapSeconds"],["bw-hide-delay","hideDelay"]].forEach(([i,k])=>{if($(i))$(i).value=data[k]});if($("bw-display-mode"))$("bw-display-mode").value=data.displayMode;if($("bw-during-giveaway"))$("bw-during-giveaway").value=data.duringGiveaway}
  function renderAll(){if(!$("bits-wheel-modal"))return;const b=data.bitsPerTier,g=data.spinsPerTier;$("bw-rule-preview").innerHTML=`<b>${b} Bits = ${g} giro${g>1?'s':''}</b><small>${b*2} Bits = ${g*2} giros · ${b*3} Bits = ${g*3} giros</small>`;const total=totalChance();$("bw-total-chance").textContent=total===100?"✅ Distribuição válida: 100%":`⚠️ Total: ${total.toFixed(2)}%`;$("bw-total-chance").className=total===100?"ok":"warn";$("bw-prize-list").innerHTML=data.prizes.map(p=>`<article class="citem bw-prize-item"><span class="bw-color" style="background:${p.color}"></span><div class="hperson"><b>${esc(p.name)}</b><small>${Number(p.chance)}% de chance</small></div><label class="bw-switch"><input type="checkbox" ${p.active?'checked':''} onchange="BitsWheel.togglePrize('${p.id}',this.checked)"><span></span></label><button class="iaction" onclick="BitsWheel.removePrize('${p.id}')">🗑️</button></article>`).join('')||'<div class="history-empty">Nenhum prêmio cadastrado.</div>';$("bw-queue-count").textContent=`${data.queue.length} evento(s) · ${data.paused?'PAUSADA':'ATIVA'}`;$("bw-queue-list").innerHTML=(current?[queueCard(current,true)]:"")+data.queue.map(q=>queueCard(q,false)).join('')+(data.lastCompleted?queueCard({...data.lastCompleted,source:"CONCLUÍDO"},false):"")||'<div class="history-empty">Fila vazia.</div>';$("bw-history-list").innerHTML=data.history.slice().reverse().map(h=>`<article class="citem"><div class="citem-head"><span class="ctrigger">@${esc(h.user)} · ${h.bits||0} Bits</span><span class="meta">${new Date(h.time).toLocaleString('pt-BR')}</span></div><div class="cmessage">${h.results.map(x=>'🎁 '+esc(x)).join('<br>')}</div><div class="metas"><span class="meta">${h.spins} giros</span><span class="meta">Entrega manual</span></div></article>`).join('')||'<div class="history-empty">Nenhum giro registrado.</div>'}
  function queueCard(q,active){return `<article class="citem ${active?'bw-running':''}"><div class="citem-head"><span class="ctrigger">@${esc(q.user)}</span><span class="meta">${active?'GIRANDO AGORA':esc(q.source)}</span></div><div class="metas"><span class="meta">${q.bits||0} Bits</span><span class="meta">${q.spins} giros</span>${q.bonus?`<span class="meta">${q.bonus} brinde</span>`:''}</div></article>`}
